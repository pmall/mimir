...
def train(args):
    ...
    # Scan dataset for unique targets
    import csv
    targets = set()
    ...
    # Resize embeddings for target tokens
    # ESM3 is likely a custom module. We need to find the embedding layer.
    # Standard HF is model.resize_token_embeddings.
    # If not, we try to locate 'embed_tokens' or similar.
    # For now, let's try the HF method, if it fails we catch it.
    try:
        # Check if resize_token_embeddings exists
        if hasattr(model, "resize_token_embeddings"):
            model.resize_token_embeddings(tokenizer.vocab_size)
            print(f"Resized model embeddings to {tokenizer.vocab_size}")
        else:
            print("WARNING: model.resize_token_embeddings not found. Attempting manual resize or skipping.")
            # If we skip, indices > vocab_size will crash.
            # We must resize.
            # Let's assume for now it works or we'll debug with explore.
            pass
